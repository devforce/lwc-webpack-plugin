...
module.exports = class Plugin {
  ...
  apply(compiler: Compiler) {
    const { namespace, modules = [] } = this.config;
    const namespaceDirectories = Object.keys(namespace).map(
      (key) => namespace[key]
    );
    ...
    compiler.options.module.rules.push({
      ...
      use: {
        ...
        options: {
          plugins: [require.resolve('@lwc/babel-plugin-component')],
      ...
      use: {
        ...
        options: {
          namespace: (path: string) => {
            ...
            const namespaceName = namespaces.find((name) =>
              path.includes(`/${name}/`)
            );
            return namespaceName || 'c';
          },
